...
class BimbinganAdmin extends StatefulWidget {
  final bool isAdmin;
  const BimbinganAdmin({super.key, this.isAdmin = false});

  @override
  State<BimbinganAdmin> createState() => _BimbinganAdminState();
}

class _BimbinganAdminState extends State<BimbinganAdmin> {
  ...
  Widget _buildShimmerBimbinganCard() {
    return Container(
  ...
  Widget _buildBimbinganCard(dynamic bimbingan) {
    String namaPeserta = bimbingan['peserta'] != null
        ? bimbingan['peserta']['nama'] ?? 'Tanpa Nama'
        : 'Tanpa Nama';
    String isiBimbingan = bimbingan['topik'] ?? '-';
    String tanggal = bimbingan['tanggal'] ?? '-';
    String rawStatus = (bimbingan['status'] ?? 'requested')
        .toString()
        .toLowerCase();
    String statusText;
    Color statusColor;
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Builder(
                            builder: (context) {
                              String? picUrl = bimbingan['peserta'] != null
                                  ? bimbingan['peserta']['profile_picture_url']
                                  : null;
                              if (picUrl == null &&
                                  bimbingan['peserta'] != null &&
  ...
  Future<void> _searchBimbingan() async {
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Button Cari Bimbingan
                    SizedBox(
                      width: double.infinity,
                      child: ElevatedButton.icon(
                        onPressed: _searchBimbingan,
                        icon: Icon(
                          Icons.search,
                          color: Colors.white,
                          size: displayWidth(context) * 0.05,
              ...
              if (_isSearching) ...[
                ...
                if (hasilPencarian.isEmpty)
                  Center(
                    child: Padding(
                      padding: EdgeInsets.all(displayWidth(context) * 0.05),
                      child: Text(
                        "Tidak ada data bimbingan untuk peserta tersebut.",
                ...
                for (var bimbingan in hasilPencarian)
                  _buildBimbinganCard(bimbingan),
              ],
